...
v1 포함으로 승격: **재고 보류**(`inv`에 재고상태 추가 → 보류 재고는 할당 후보에서 제외), **로케이션 용량 제약**(적치 전략의 전제 — 형태는 미결정).
...
stateDiagram-v2
    ...
    RECEIVING --> COMPLETED : 입고확정 (결품 확정 + 종료)
    COMPLETED --> [*]
    CANCELLED --> [*]
```

- 라인에 `expectedQty` / `receivedQty` / `putawayQty`를 두고 부분입고는 수량으로만 표현.
- **중간 상태 `RECEIVED`를 두지 않는다**: "검수는 끝났고 적치가 남은" 구간을 상태로 표현하려던 것인데, 그 진행도는 전 라인의 `rcvdQty` / `putawayQty` 비교로 항상 파생 가능하다. 헤더 상태는 워크플로 단계만 표현한다는 원칙(위)을 입고에도 그대로 적용해 `RECEIVING → COMPLETED` 한 번의 전이로 줄인다.
- **입고확정은 흐름의 마지막 단계이며, 자동 전이가 아니라 명시적 액션이다**: 전량 검수됐다고 해서 입고가 끝난 게 아니다 — 실물이 보관 로케이션에 자리를 잡아야(적치 완료) 입고가 완결된다. 검수만 하고 확정하는 것은 업무적으로 성립하지 않으므로 자동 전이를 두지 않는다.
  - 확정 시점에 확정되는 것은 **결품**(`expctQty - rcvdQty`)이다 — "예정 100개 중 90개만 도착, 나머지 10개는 안 옴"을 확정하고 입고 건을 닫는다.
  - **선행 조건: 전 라인 `putawayQty == rcvdQty`** (미적치 잔량 0). 미적치인 채로 확정을 허용하면 `RCV-STAGE`에 실물 재고가 남았는데 입고 문서는 닫혀, 그 재고를 옮길 화면 진입점이 사라진다. 결품(안 온 물건)과 미적치(왔는데 안 옮긴 물건)는 다른 문제이며, 확정은 전자만 처리한다.
...
- **적치는 헤더 상태와 무관하게 즉시 가능**: 스테이징에 쌓인 수량(`rcvdQty - putawayQty`)이 있으면 아직 입고확정 전이라도 적치할 수 있다 — 적치를 미룰 이유가 없기 때문. 검수와 적치는 순서가 아니라 수량으로 맞물리며, 둘 다 `RECEIVING` 구간 안에서 병행된다.
- **적치 로케이션 배정은 전략 기반 자동 배정**: 적치지시 생성 시 전략(적용 조건 + 우선순위 + 배정 방식)에 따라 로케이션을 정한다. 전제로 로케이션 용량 제약이 필요하다 — 용량이 없으면 후보가 "온도대 일치 STORAGE 전부"라 전략이 정렬 순서밖에 바꾸지 못한다. **미결정: 용량 제약의 형태(수량 상한 / 1로케이션 1SKU)** 를 정한 뒤 전략 항목을 확정한다.
...
- **적치 지시(putaway task) 단계를 둔다**: 전략이 정한 배정 결과를 담을 레코드가 필요하기 때문이다. 지시 생성(전략 적용) 과 지시 실행(실물 MOVE) 을 분리해, 작업자는 지시를 보고 움직이고 시스템은 지시 대비 실적을 추적한다. **미결정 2건**: ① 지시와 다른 로케이션에 적치한 경우 허용 여부(허용하면 지시는 권고, 막으면 명령) ② 지시 생성 시 로케이션을 선점할지(선점 시 용량 제약과 맞물려 출고 할당과 같은 동시성 경합이 생긴다).
...
- 조건: ① 주문이 `RECEIVING` 상태일 때만(입고확정 이후 불가 — 확정은 명시적 종료라는 원칙과 충돌) ② 취소 수량만큼 해당 Lot의 스테이징 `onHandQty`가 남아있어야 함(이미 적치된 분은 취소 불가).
...
- **피킹지시 엔티티를 둔다**: 웨이브에 묶인 주문들의 할당 레코드를 모아 로케이션 순으로 정렬한 지시를 발행한다. 지시 행은 **할당 레코드와 1:1**이며, SKU×로케이션으로 뭉치는 배치 피킹은 도입하지 않는다 — 뭉치면 집품 후 주문별 분류 공정이 추가로 필요해지는데 v1은 그 복잡도를 회피한다. 웨이브가 주는 이점은 "여러 주문의 집품 동선을 한 번에 정렬"까지다.
...
- **점포 잔여수명 필터가 FEFO 앞단에 붙는다**: 주문 점포의 `store.outb_life_rate`로 Lot을 먼저 거른 뒤 FEFO 정렬한다 — 같은 SKU라도 A점포엔 되는 Lot이 B점포엔 안 될 수 있다 — 같은 SKU라도 A점포엔 되는 Lot이 B점포엔 안 될 수 있다. FEFO 정렬 **앞단의 필터**로 동작한다.
