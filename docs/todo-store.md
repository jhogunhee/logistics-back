...
- [ ] **store_cd를 누가 만드나?** 상품처럼 서버 채번(시퀀스)인지, Loc처럼 사용자 입력+중복검증인지.
...
- [ ] 라우터/메뉴에 StoreMaster 등록 (기존 상품/Loc 메뉴 등록 위치 참고)
...
- [ ] 커밋 (간결한 메시지로)
- [ ] 여유 있으면: 상품/Loc/Store 세 화면의 중복을 보고 "이제는 공통화할 때인가?"를 스스로 판단해보기 — 세 번 반복됐으면 후보다
