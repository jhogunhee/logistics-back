...
-- =====================================================================
-- 2. OMS 주문 OMS_* (CREATED → CONVERTED / CANCELLED)
--    주문 원장. WMS 작업문서(입고예정 ASN)의 유일한 발생지다.
--    변환(convert) 시 같은 트랜잭션에서 ib_order를 생성한다 — ASN 직접 등록 경로는 없다.
--    변환취소하면 ASN은 CANCELLED로 남고 주문은 CREATED로 돌아와 재변환할 수 있다.
-- =====================================================================
...
COMMENT ON TABLE  oms_ib_order IS '입고주문(벤더 발주) 헤더. 변환 시 ib_order(ASN)를 생성하는 상위 원장. ASN의 생성·취소는 전적으로 여기서 주관한다';
...
COMMENT ON TABLE  oms_ib_line IS '입고주문 라인. 변환 시 ib_line으로 1:1 복사된다 (odr_qty를 출고단위로 환산 → expct_qty)';
COMMENT ON COLUMN oms_ib_line.odr_qty IS '발주 수량. <<입고단위(prod.inb_uom_cd) 기준>> — DB에서 출고단위가 아닌 유일한 수량 컬럼이다. ASN 변환 시 prod_uom.ea_qty(입고단위)를 곱하고 ea_qty(출고단위)로 나눠 환산된다';
...
-- OMS 입고주문 변환으로만 생성된다 (등록·취소 엔드포인트가 WMS에 없다).
...
-- 주문 하나가 붙들 수 있는 "유효한" ASN은 하나. 변환취소는 행을 지우지 않고 CANCELLED로 남기므로
-- (언제 왜 물렸는지 추적용) 취소분을 제외한 부분 유니크로 표현한다. 재변환하면 새 행이 하나 더 생긴다.
-- 동시 변환 요청 시 두 트랜잭션이 모두 CREATED를 읽고 통과하는 구간을 DB가 막는 최후 방어선.
