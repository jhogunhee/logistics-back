...
COMMENT ON TABLE  vendor IS '벤더(납품처) 마스터. 입고주문·입고예정의 거래처. 거래 종료는 삭제가 아니라 us_yn=N';
COMMENT ON COLUMN vendor.vndr_cd IS '벤더 코드 (업무 식별자, 예: VD-0001). 서버가 시퀀스로 채번';
COMMENT ON COLUMN vendor.us_yn  IS '사용 여부. N이면 신규 주문에서 선택 불가 — 과거 주문의 참조는 그대로 유지된다';
...
CREATE TABLE code_detail (
    ...
    CONSTRAINT ck_code_us_yn CHECK (us_yn IN ('Y', 'N'))
);

COMMENT ON TABLE  code_detail IS '공통코드 상세. 그룹 내 개별 코드와 표시명/정렬 순서';
COMMENT ON COLUMN code_detail.code_cd  IS '코드 값 (예: DRY). 로직에서 리터럴로 참조하므로 변경 금지, 폐기는 us_yn=N';
COMMENT ON COLUMN code_detail.us_yn   IS '사용 여부. 과거 데이터가 참조하므로 삭제 대신 N 처리';
...
COMMENT ON TABLE  oms_ib_line IS '입고주문 라인. 변환 시 ib_line으로 1:1 복사된다 (odr_qty → expct_qty)';
COMMENT ON COLUMN oms_ib_line.odr_qty IS '발주 수량. 그대로 ASN 라인의 입고 예정 수량이 된다';
