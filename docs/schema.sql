...
COMMENT ON COLUMN ptawy_stgy.prty        IS '낮을수록 우선. 동률 허용 — 선택은 (prty, ptawy_stgy_id) 순으로 결정적, 동률 경고는 화면·저장 서비스가 담당';
COMMENT ON COLUMN ptawy_stgy.tgt_cond    IS '적용대상 조건 [{fld,op,vals}] — 빈 배열 = 전체 매칭. 필드는 putaway-target 레지스트리(TMP_ZON·VNDR·PROD). 구조는 전략_테이블설계서.md §8.3';
...
COMMENT ON COLUMN ptawy_stgy_stg.mthd_cd   IS '추천 방식 code (1차: SAME_PROD_LOC 적재 / EMPTY_LOC 빈 / ANY_LOC 전체 보관). 레거시 FIXED_LOC은 고정상품 마스터가 없어 제외. CHECK 없음 — 레지스트리가 소유';
COMMENT ON COLUMN ptawy_stgy_stg.mthd_para IS '방식 파라미터 (1차 방식들은 빈 객체 — 확장 대비)';
COMMENT ON COLUMN ptawy_stgy_stg.line_cond IS '이 단계를 시도할 입고라인 조건. 빈 배열 = 항상 시도';
COMMENT ON COLUMN ptawy_stgy_stg.loc_cond  IS '후보 로케이션 범위 조건 (ZON·LOC_CD). 빈 배열 = 제한 없음. 상품 온도대 일치 + STORAGE는 조건이 아니라 모든 단계의 불변 전제라 저장하지 않는다';
