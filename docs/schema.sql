...
--    작성자는 JPA Auditing(AuditorAware)이 채운다 (로그인 아이디, 스케줄러 실행은 'system')
--
...
-- 상품 거래처 마스터 (자동발주 기준). 상품×벤더 — 그 벤더에서 그 상품을 살 때의 발주점·상한·MOQ·리드타임.
-- prod에 컬럼으로 두지 않은 이유: 리드타임과 최소주문수량은 상품의 성질이 아니라 거래 조건이라
-- 벤더가 둘이면 두 벌이다. 한 상품이 여러 벤더를 갖는 것은 허용하고(prty로 대표를 고른다),
-- 한 벤더가 여러 상품을 대는 것도 당연히 허용한다 — 그래서 UNIQUE는 (prod_id, vendor_id) 짝이다.
-- min_qty/max_qty는 낱개(EA) 기준이고(창고 수량과 비교한다) min_odr_qty만 입고단위다(발주 수량의 단위).
...
CREATE INDEX ix_prod_vndr_vendor ON prod_vndr (vendor_id); -- 벤더→상품 조회 (벤더 삭제 가드)
...
-- 사용자 마스터. 로그인과 역할이 여기서 나온다.
-- 역할 목록은 별도 마스터가 아니라 코드(enum)에 고정한다 — 역할이 곧 URL 접두(업무 구역)라
-- 데이터로 늘려도 그 접두를 아는 코드가 함께 바뀌어야 하고, 그러면 두 벌을 맞추는 일만 남는다.
CREATE TABLE usr (
...
-- 사용자 역할. 한 사용자가 여럿을 갖는다 — 작은 센터는 한 명이 입고+재고를 겸하는데
-- 단일 역할이면 「입고재고담당」 같은 조합 역할이 계속 생긴다. 판정은 「하나라도 있으면 통과」.
...
INSERT INTO code_detail (grp_cd, code_cd, code_nm, srt_seq) VALUES ('ODR_DVSN', 'ATO', '자동발주', 4);
