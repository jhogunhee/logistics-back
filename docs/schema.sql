...
CREATE TABLE prod (
    ...
    ib_life_rate    SMALLINT,
    created_at      TIMESTAMP       DEFAULT CURRENT_TIMESTAMP NOT NULL,
    created_by      VARCHAR(30)     DEFAULT 'admin' NOT NULL,
    updated_at      TIMESTAMP,
    updated_by      VARCHAR(30),
    CONSTRAINT uq_prod_cd UNIQUE (prod_cd),
    CONSTRAINT ck_prod_temp_zone CHECK (temp_zone IN ('DRY', 'CHL', 'FRZ')),
    CONSTRAINT ck_prod_ib_life_rate CHECK (ib_life_rate IS NULL OR ib_life_rate BETWEEN 0 AND 100)
);
...
CREATE TABLE loc (
    ...
    CONSTRAINT ck_loc_storage_capacity CHECK (loc_type <> 'STORAGE' OR max_qty IS NOT NULL)
);
...
COMMENT ON COLUMN loc.pick_prty IS '할당 시 동일 유통기한(FEFO 동순위) 간 로케이션 우선순위. 낮을수록 먼저 할당. 적치 지시의 후보 순회 순서이기도 하다';
...
COMMENT ON COLUMN oms_ib_order.expct_dt     IS '입고 예정일. ASN의 입고번호 채번(IB-YYYYMMDD-NNN) 기준일이기도 하다';
...
COMMENT ON COLUMN oms_ib_line.order_qty IS '발주 수량. 그대로 ASN 라인의 입고 예정 수량이 된다';
...
COMMENT ON TABLE  outb_alloc IS '재고 할당 레코드. 어떤 주문라인이 어떤 재고(상품+Loc+Lot)를 몇 개 예약했는지. 할당 취소 시 삭제 + inv.alloc_qty 복원';
