...
--
--   ▣ 리셋 스크립트가 둘이다. 이 파일은 <상품까지 지운다> — 마스터를 지키고 싶으면
--     reset-dev-keep-master.sql 을 쓸 것 (상품 · 포장 · 로케이션 · 벤더를 남긴다).
--
--   지우는 것 : 입고주문 · ASN · 검수 · 적치지시 · 적치 · 재고 · 이력 · 이동지시 · 보류 · 재고조사 · 출고 · Lot · 상품 · 포장
--   남기는 것 : 공통코드(UOM 그룹 포함) · 존 · 로케이션 · 벤더 · 점포 · 사용자(usr · usr_role)
--
...
BEGIN
    ...
    -- putaway_task(적치지시)는 스테이징 재고를 aloc 로 예약하므로 inv 와 함께 비워야 한다 —
    -- 지시만 남으면 예약 원천이 사라진 유령 예약이 된다.
