...
- **그 짝을 묶는 자리가 `inventory.service.InvStore`다.** 스냅샷 증감 · 이력 기록 · 수량이 모두 0이 된 행 삭제 셋을 한 메서드로 처리한다(`increase` / `decrease` / `move` / `reserve` / `release` / `hold` / `releaseHold`, 이력의 문서 참조 컬럼은 `InvDocRef`). **서비스에서 `Inv`의 증감 메서드나 `invRepository.save` · `delete`를 직접 부르지 말 것** — 그 셋을 손으로 맞추다 조건이 갈라진 적이 있어 포트로 모았다. 락은 잡는 순서로 교착을 피하는 서비스 쪽 책임으로 남아 있다.
