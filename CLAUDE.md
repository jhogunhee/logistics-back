...
**화면프로세스 정의서와 전략 설계자료는 이 레포에 두지 않는다** — 레포 밖 `../docs/{ob,sg,st}/`에 있다(코드와 함께 관리할 문서가 아니라 참고 자료다). 위 문서들과 DB 코멘트가 `docs/st/전략_테이블설계서.md` 같은 이름으로 인용하는 것들이 그것이고, 경로가 아니라 **문서 이름**으로 읽으면 된다.
