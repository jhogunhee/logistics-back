 ...
 *
 * <p>판정에 쓰이는 값은 {@code outbTyp}·{@code vhclFltno}·{@code storeGrp}·{@code storeTyp}
 * 넷이고, 나머지는 화면·로그에 「어느 주문이 왜 편입/제외됐는지」를 보여주기 위한 표시용이다.
 */
public record WaveOrderTarget(
        ...
        String storeCd,
        String storeNm,
        /** 점포그룹·점포유형 — 주문이 아니라 점포 마스터의 값이다. null = 미지정 (부정 연산자만 참) */
        String storeGrp,
        String storeTyp,
        LocalDate expctDe
) {
    ...
    public static WaveOrderTarget from(OutbOrder order) {
        return new WaveOrderTarget(order.getId(), order.getOutbNo(),
                ...
                order.getStore().getStoreGrp(), order.getStore().getStoreTyp(), order.getExpctDe());
    }
}
