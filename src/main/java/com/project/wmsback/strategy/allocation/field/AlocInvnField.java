 ...
 *
 * <p><b>주의 — 이 축은 켜는 순간 후보 모집단을 줄인다.</b> 계층 미지정일 때의 후보는 보관존과
 * 피킹존을 <b>모두</b> 포함한다. {@code BIZ_DVSN IN [PIKNG]} 하나만 걸면 보관존 재고가 후보에서
 * 빠지는데, <b>보충(보관 → 피킹존) 프로세스가 없어</b> 그 물량은 결품(부족 출고)이 된다.
 * 어느 계층에도 안 걸린 후보는 끝내 배정되지 않으므로, 조건 없는 계층을 마지막에 두어 받아내야 한다.
 */
