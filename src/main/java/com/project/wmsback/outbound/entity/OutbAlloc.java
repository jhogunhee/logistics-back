...
public class OutbAlloc extends BaseEntity {
    ...
    /**
     * 보충 확정이 할당을 도착지 재고 행으로 옮긴다 — 예약이 실물을 따라갔으니 할당도 따라간다.
     * 이 뒤의 피킹·결품 종결·할당해제는 전부 새 행을 본다. 피킹이 시작된 할당은 옮길 수 없다
     * (보충은 피킹 전에만 있다 — 실행 가드가 보장하지만 여기서도 막는다).
     */
    public void relocate(Inv toInv) {
        if (pikngQty != 0L) {
            throw new IllegalStateException("피킹이 시작된 할당은 재고 행을 옮길 수 없습니다 (기피킹 " + pikngQty + ")");
        }
        this.inv = toInv;
    }
