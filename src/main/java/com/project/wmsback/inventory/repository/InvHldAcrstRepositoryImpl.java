...
import static com.project.wmsback.inventory.entity.QInvHldAcrst.invHldAcrst;
...
public class InvHldAcrstRepositoryImpl implements InvHldAcrstRepositoryCustom {
    ...
    public PageResponse<InvHldAcrstResponse> search(InvHldAcrstSearchCond cond, PageCond pageCond) {
        List<InvHldAcrstResponse> rows = queryFactory
                .select(Projections.constructor(InvHldAcrstResponse.class,
                        invHldAcrst.id, invHldAcrst.hldNo,
                        prod.prodCd, prod.prodNm,
                        loc.locCd, lot.lotNo,
                        invHldAcrst.hldQty,
                        invHldAcrst.rsnCd, invHldAcrst.rsnDscr,
                        invHldAcrst.createdAt))
                .from(invHldAcrst)
                .innerJoin(invHldAcrst.prod, prod)
                .innerJoin(invHldAcrst.loc, loc)
                .innerJoin(invHldAcrst.lot, lot)
                .where(searchConds(cond))
                .orderBy(invHldAcrst.id.desc())
                .offset(pageCond.getOffset())
                .limit(pageCond.getSize())
                .fetch();

        Long totCnt = queryFactory
                .select(invHldAcrst.count())
                .from(invHldAcrst)
                .innerJoin(invHldAcrst.prod, prod)
                .innerJoin(invHldAcrst.loc, loc)
                .innerJoin(invHldAcrst.lot, lot)
    ...
    private BooleanExpression hldNoContains(String hldNo) {
        return StringUtils.hasText(hldNo) ? invHldAcrst.hldNo.containsIgnoreCase(hldNo) : null;
    }
    ...
    private BooleanExpression rsnCdEq(String rsnCd) {
        return StringUtils.hasText(rsnCd) ? invHldAcrst.rsnCd.eq(rsnCd) : null;
    }

    private BooleanExpression createdAtGoe(LocalDate dateFrom) {
        return dateFrom != null ? invHldAcrst.createdAt.goe(dateFrom.atStartOfDay()) : null;
    }

    private BooleanExpression createdAtLt(LocalDate dateTo) {
        return dateTo != null ? invHldAcrst.createdAt.lt(dateTo.plusDays(1).atStartOfDay()) : null;
    }
}
